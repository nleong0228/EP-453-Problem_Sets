...
class MultiTouchSynth {
    ...
    init(){
        for _ in 0..<ChorusController.numFingers {
            add()
        }
    }
    ...
    func add(){
        ...
        let chorus = Chorus(osc)
        let filter = LowPassFilter(chorus)
        ...
        nodes.append([osc, filter, env, chorus, reverb])
    }
    ...
    func update(id: Int, point: CGPoint){
        ...
        let rate = scale(num: point.y, minNum: 0, maxNum: 1, scaleMin: 25, scaleMax: 100)
        let depth = scale(num: point.x, minNum: 0, maxNum: 1, scaleMin: 0, scaleMax: 0.5)
        let offset = scale(num: point.y, minNum: 0, maxNum: 1, scaleMin: 0, scaleMax: 0.85)
        let multiplier = scale(num: point.x, minNum: 0, maxNum: 1, scaleMin: 5, scaleMax: 20)
        let synths = nodes[ids[id]!]
        ...
        osc.detuningOffset = AUValue(offset)
        osc.detuningMultiplier = AUValue(multiplier)
        let filter = synths[1] as! LowPassFilter
        filter.cutoffFrequency = AUValue(freq)
        let chorus = synths[3] as! Chorus
        chorus.frequency = AUValue(rate)
        chorus.depth = AUValue(depth)
    }
